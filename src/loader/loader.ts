...
export function githubDiscussionsBlogLoader({
    ...
    return {
        ...
        load: async ({ store, parseData, generateDigest, meta, renderMarkdown, logger }): Promise<void> => {
            ...
            const processor = githubPostProcessor(renderMarkdown);

            for (const item of posts) {

                const { post, rendered } = await processor.process(item);
